...
module ay8913 #(parameter DA7_DA4_UPPER_ADDRESS_MASK = 4'b0000,
                parameter CHANNEL_OUTPUT_BITS = 8  // digital output quality
) (
    input  wire       ena,          // will go high when the design is enabled
    input  wire       clk,          // clock
    input  wire       rst_n,        // reset_n - low to reset

    input  wire [1:0] clock_select, // SEL in YM2149, not present in AY-3-819x

    input  wire [7:0] data,
    input  wire bdir,
    input  wire bc1,

    output wire[14:0] channel_A_dac_ctrl,
    output wire[14:0] channel_B_dac_ctrl,
    output wire[14:0] channel_C_dac_ctrl,

    output wire channel_A_pwm_out,
    output wire channel_B_pwm_out,
    output wire channel_C_pwm_out
);
    initial begin
        $dumpfile ("ay8913.vcd");
        $dumpvars (0, ay8913);
    ...
    wire cs = (data[7:4] == DA7_DA4_UPPER_ADDRESS_MASK) && ena;
    ...
    wire [CHANNEL_OUTPUT_BITS-1:0] channel_A_pwm_ctrl, channel_B_pwm_ctrl, channel_C_pwm_ctrl;
    amplitude #(.VOLUME_BITS(CHANNEL_OUTPUT_BITS)) amp_A (
        .in(channel_A),
        .control(envelope_A ? envelope: amplitude_A),
        .pwm_out(channel_A_pwm_ctrl),
        .dac_out(channel_A_dac_ctrl)
        );
    amplitude #(.VOLUME_BITS(CHANNEL_OUTPUT_BITS)) amp_B (
        .in(channel_B),
        .control(envelope_B ? envelope: amplitude_B),
        .pwm_out(channel_B_pwm_ctrl),
        .dac_out(channel_B_dac_ctrl)
        );
    amplitude #(.VOLUME_BITS(CHANNEL_OUTPUT_BITS)) amp_C (
        .in(channel_C),
        .control(envelope_C ? envelope: amplitude_C),
        .pwm_out(channel_C_pwm_ctrl),
        .dac_out(channel_C_dac_ctrl)
        );
    ...
    pwm #(.VALUE_BITS(CHANNEL_OUTPUT_BITS)) pwm_A (
        .clk(clk),
        .reset(reset),
        .value(channel_A_pwm_ctrl),
        .out(channel_A_pwm_out)
        );

    pwm #(.VALUE_BITS(CHANNEL_OUTPUT_BITS)) pwm_B (
        .clk(clk),
        .reset(reset),
        .value(channel_B_pwm_ctrl),
        .out(channel_B_pwm_out)
        );

    pwm #(.VALUE_BITS(CHANNEL_OUTPUT_BITS)) pwm_C (
        .clk(clk),
        .reset(reset),
        .value(channel_C_pwm_ctrl),
        .out(channel_C_pwm_out)
        );
